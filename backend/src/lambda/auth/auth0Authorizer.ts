...
const jwksUrl = 'https://pslinux.eu.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const signingKeys = jwks.filter(key => key.use === 'sig' // JWK property `use` determines the JWK is for signing
